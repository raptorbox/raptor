 ...
 */
public class DevicePermissionClient extends AbstractClient {

    public DevicePermissionClient(Raptor container) {
        super(container);
    }

    final static Logger logger = LoggerFactory.getLogger(DevicePermissionClient.class);
    ...
    public List<String> set(String tokenId, List<String> permissions) {
        ...
        JsonNode node = getClient().put(String.format(Routes.PERMISSION_SET, "device", tokenId), toJsonNode(req));
        return getMapper().convertValue(node, new TypeReference<List<String>>() {
        });
    }
